...
function CheckoutContent() {
  ...
  // Se a cliente veio da página da Caixa Misteriosa com observações
  // preenchidas, já traz elas prontas pro campo de observações do pedido.
  useEffect(() => {
    const notesFromMysteryBox = sessionStorage.getItem('checkoutNotes');
    if (notesFromMysteryBox) {
      setAddress((prev) => ({ ...prev, notes: notesFromMysteryBox }));
    ...
    <div className="pt-32 pb-24 max-w-3xl mx-auto px-6">
      ...
      {step === 'endereco' && (
        <form onSubmit={handleAddressSubmit} className="bg-white p-8 rounded-sm shadow-[0_6px_20px_rgba(78,22,38,0.06)] grid sm:grid-cols-2 gap-5">
          ...
          <label className="label-field sm:col-span-2">
            Cor de preferência e observações (opcional)
            <textarea
              className="input-field min-h-[80px]"
              placeholder="Ex: prefiro tons de vermelho ou marsala, e qualquer outra informação sobre o pedido..."
              value={address.notes}
